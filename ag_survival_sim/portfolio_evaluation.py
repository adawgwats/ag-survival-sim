...
from .evaluation import PolicyMetrics
...
@dataclass(frozen=True)
class PortfolioPolicyEvaluation:
    policy_name: str
    path_results: list[PortfolioPathResult]


@dataclass(frozen=True)
class PortfolioScenarioEvaluationSummary:
    evaluations: dict[str, PortfolioPolicyEvaluation]
...
def evaluate_portfolio_policies(
    *,
    ...
    horizon_years: int,
    num_paths: int,
) -> PortfolioScenarioEvaluationSummary:
    evaluations: dict[str, PortfolioPolicyEvaluation] = {}
    metrics: dict[str, PolicyMetrics] = {}
    ...
    for policy_name, policy in policies.items():
        ...
        for path_index in range(num_paths):
            ...
            path_results.append(PortfolioPathResult(steps=steps))

        evaluations[policy_name] = PortfolioPolicyEvaluation(policy_name=policy_name, path_results=path_results)
        metrics[policy_name] = summarize_portfolio_policy(path_results)

    return PortfolioScenarioEvaluationSummary(evaluations=evaluations, metrics=metrics)


def summarize_portfolio_policy(path_results: list[PortfolioPathResult]) -> PolicyMetrics:
    if not path_results:
