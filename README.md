...
- separate diversified portfolio benchmark with quantity allocations across multiple crops
...
## DSSAT-backed diversified portfolio benchmark

The package now also includes a separate portfolio benchmark that lets a policy allocate acreage across multiple crop options in the same farm-year.

Current portfolio bundle:

- `georgia_diversified_portfolio`
  - corn from `georgia_maize_management`
  - soybean from `georgia_soybean`
  - peanut from `georgia_peanut`

This benchmark is distinct from the single-action benchmark:

- a policy chooses acreage by crop/management option
- total planted acres must stay within farm acreage
- operating costs must still fit available cash plus credit
- DSSAT is run through the underlying crop-specific benchmark bundles and the results are aggregated through the same finance engine

CLI:

```bash
ag-survival-portfolio-benchmark --benchmark georgia_diversified_portfolio --paths 6 --horizon 6
```
